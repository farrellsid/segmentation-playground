...
def parse_chains(spec: str) -> list[tuple[str, int]]:
    ...
    for part in spec.split(","):
        ...
        if not part:
            continue
        if part.count(":") != 1:
            raise ValueError(f"bad --chains entry {part!r}; expected NEURON:chain_idx (e.g. AIAL:5)")
...
RESULTS_START = "<!-- bakeoff:results (auto-generated by sam3_bakeoff.py; do not edit inside) -->"
RESULTS_END = "<!-- /bakeoff:results -->"


def append_findings_doc(rows: list[dict], path: Path = FINDINGS_DOC) -> None:
    """Write the results table into the findings doc between stable marker comments.

    Idempotent: a re-run REPLACES the marked block rather than appending a duplicate
    (the doc is version-controlled, and this harness is meant to be re-run for the
    broader multi-neuron sweep). On the very first run (markers absent) it fills the
    placeholder; if neither is present it appends the marked block once.
    """
    ...
    print(f"[bakeoff] wrote results table -> {path}")
