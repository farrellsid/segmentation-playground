"""
labels.py: the review GUI's per-frame label store (the "label engine").
...
row) so a later predictor can train a learned ``P(error)`` detector on
the exhaust of review (the GUI as a label engine).

The split is deliberate:
    here        COLLECTS labels: the GUI appends rows as the human works.
    later       TRAINS on them: logistic / small-GBT over the signal vector.
This module is the collection half only. It does no modelling, no thresholding, no
...
labeling was too much effort up front). Its two structural facts still bind label
collection, and this store enforces them:
  ...
  1. **Anchor contamination poisons labels.** A frame that is wrong because the
     *anchor* was wrong is not a propagation-signal failure; training on it teaches
     ...
     => every row carries the chain's anchor verdict (``anchor_passed`` /
        ``anchor_reasons`` / ``anchor_contained``) so the trainer can exclude or separately
  ...
  2. **Selection bias is the killer.** Labels collected *only* on flagged frames are
     ...
     "good" frames too. Non-negotiable; without it the eventual model only
...
anchor verdict anchor_passed, anchor_reasons, anchor_contained  (chain-level,
              repeated on every row of the chain: the anchor-contamination guard)
...
def _atomic_write_csv(df: pd.DataFrame, path: Path) -> None:
    """Write via a temp file in the same dir, then os.replace, so a crash mid-write
    can't corrupt the store (same pattern as batch._atomic_write_csv). Important
    once the GUI and a background batch can both touch the output tree (parallel
    review)."""
    path = Path(path)
...
class LabelStore:
    ...
    produces labels slowly; data volume is not the constraint, label coverage
    is).
    """
    ...
    def sample_unflagged(
        ...
        *,
        ...
        This is the selection-bias guard made concrete: the only window onto
        ...
        rate the eval set estimates. The reviewer is expected to actually look
