...
<div class="mt-2 text-center opacity-80">URAVL, one slice, a real fix: the SAM2 mask swallows a neighbour's node (red); the SAM3 mask stays in its own cell.</div>
...
- left: SAM2 mask fuses two adjacent profiles, swallows the neighbour's skeleton node (red marker)
- right: SAM3 mask sits inside the correct cell, that bleed is gone
